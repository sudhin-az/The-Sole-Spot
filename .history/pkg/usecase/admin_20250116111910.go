...
func (ad *AdminUseCase) ChangeOrderStatus(orderID int, status string) (models.Order, error) {
	orderStatus, err := ad.adminrepository.ChangeOrderStatus(orderID, status)
	if err != nil {
	...
	return models.Order{
		...
		AddressID:       orderStatus.AddressID,
		CouponID:        orderStatus.CouponID,
		Discount:        orderStatus.Discount,
		GrandTotal:      orderStatus.GrandTotal,
		Status:          status,
		Method:          orderStatus.Method,
		PaymentStatus:   orderStatus.PaymentStatus,
		PaymentMethodID: orderStatus.PaymentMethodID,
		OrderDate:       orderStatus.OrderDate,
		DeliveryTime:    orderStatus.DeliveryTime,
		ShipmentStatus:  orderStatus.ShipmentStatus,
		Approval:        orderStatus.Approval,
		FinalPrice:      orderStatus.FinalPrice,
	}, nil
}
