...
func (ad *AdminUseCase) GetOrderDetails(userID int) ([]models.FullOrderDetails, error) {
	...
	if err != nil {
		fmt.Println("Error in repository:", err) // Debugging
		return nil, err
	}
	if len(fullOrderDetails) == 0 {
		fmt.Println("No full order details found for user_id:", userID) // Debugging
	}
...
func (ad *AdminUseCase) ChangeOrderStatus(orderID string) (models.Order, error) {
	shipmentStatus, err := ad.adminrepository.ChangeOrderStatus(orderID)
	if err != nil {
