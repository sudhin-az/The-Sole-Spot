...
func (o *OrderUseCase) OrderItemsFromCart(order models.Order) (models.Order, error) {
	...
	for _, item := range cartItems {
		order.GrandTotal = item.TotalPrice - order.Discount
	}
	...
	for _, item := range cartItems {
		...
		if err != nil {
			return models.Order{}, fmt.Errorf("failed to fetch stock for product %d: %w", item.ProductID, err)
		}
		if item.Quantity > availableStock {
			return models.Order{}, fmt.Errorf("insufficient stock for product ID %d; please remove it from the cart", item.ProductID)
		}
		...
		if err != nil {
			return models.Order{}, fmt.Errorf("failed to update stock for product %d: %w", item.ProductID, err)
		}
	}
