...
func (o *OrderUseCase) OrderItemsFromCart(order models.Order, cartItems []models.Cart) (domain.OrderSuccessResponse, error) {
	...
	addressExist, err := o.orderRepository.AddressExist(models.OrderIncoming{})
	if err != nil {
		return domain.OrderSuccessResponse{}, err
	}
	if !addressExist {
	...
	for _, item := range cartItems {
		...
		err = o.orderRepository.UpdateProductStock(item.ProductID, newStock)
		if err != nil {
	...
	// Save order items
	err = o.orderRepository.CreateOrderItems(orderItems)
	if err != nil {
	...
	// Get brief order details to return
