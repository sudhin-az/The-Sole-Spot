...
func (ad *AdminUseCase) CancelOrders(orderID string, userID int) error {
	...
	orderStatus, err := ad.adminrepository.Getshipmentstatus(orderIDInt)
	if err != nil {
		return err
	}
...
func (ad *AdminUseCase) ChangeOrderStatus(orderID string, status string) (models.Order, error) {
	order, err := ad.adminrepository.ChangeOrderStatus(orderID, status)
	if err != nil {
		return models.Order{}, fmt.Errorf("could not update status: %w", err)
	}
	return order, nil
}
