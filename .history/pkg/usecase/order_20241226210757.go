...
func (o *OrderUseCase) OrderItemsFromCart(order models.Order, cartItems []models.OrderFromCart) (domain.OrderSuccessResponse, error) {
	...
	err := copier.Copy(&orderBody, &cartItems)
	if err != nil {
		return domain.OrderSuccessResponse{}, err
	}
	orderBody.UserID = orderBody.UserID
	...
	for _, c := range cartItems {
		...
		fmt.Println("p_id", c.ProductID)
		availableStock, err := o.orderRepository.GetProductStock(c.ProductID)
		if err != nil {
			return domain.OrderSuccessResponse{}, err
		}
		fmt.Println("Quantity:", c.Quantity)
		fmt.Println("Stock:", availableStock)
		if c.Quantity > availableStock {
			return domain.OrderSuccessResponse{}, fmt.Errorf("Insufficient Stock for the product_ID %d, pls remove this from cart.", c.ProductID)
		}
