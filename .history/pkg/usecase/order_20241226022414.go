...
func NewOrderUseCase(orderRepository repository.OrderRepository, userRepository repository.UserRepository, cartRepository repository.CartRepository, productRepository repository.ProductRepository) *OrderUseCase {
	return &OrderUseCase{
...
func (o *OrderUseCase) OrderItemsFromCart(orderFromCart models.OrderFromCart, userID int) (domain.OrderSuccessResponse, error) {
	...
	for _, c := range cartItems {
		...
		availableStock, err := o..GetProductStock(c.ProductID)
		if err != nil {
			return domain.OrderSuccessResponse{}, err
		}
		if c.Quantity > availableStock {
			return domain.OrderSuccessResponse{}, fmt.Errorf("product ID %d is out of stock, only %d items available", c.ProductID, availableStock)
		}
	}
