...
func (o *OrderUseCase) OrderItemsFromCart(order models.Order) (domain.Order, error) {
	cartExist, err := o.orderRepository.DoesCartExist(order.UserID)
	if err != nil {
		return domain.Order{}, err
	}
	if !cartExist {
		return domain.Order{}, errors.New("cart is empty; cannot place order")
	}
	...
	if !addressExist {
		return domain.Order{}, errors.New("address does not exist")
	}
	...
	for _, item := range cartItems {
		...
		if err != nil {
			return domain.Order{}, fmt.Errorf("failed to fetch stock for product %d: %w", item.ProductID, err)
		}
		if item.Quantity > availableStock {
			return domain.Order{}, fmt.Errorf("insufficient stock for product ID %d; please remove it from the cart", item.ProductID)
		}
		...
		if err != nil {
			return domain.Order{}, fmt.Errorf("failed to update stock for product %d: %w", item.ProductID, err)
		}
	}
	...
	if err != nil {
		return domain.Order{}, fmt.Errorf("failed to create order items: %w", err)
	}
	...
	if err != nil {
		return domain.Order{}, fmt.Errorf("failed to fetch brief order details: %w", err)
	}
