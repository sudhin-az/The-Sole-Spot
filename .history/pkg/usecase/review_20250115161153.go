...
func (r *ReviewUseCase) AddReview(userID int, productID string, Rating float64, Comment string) (models.ReviewResponse, error) {
	...
	if err != nil {
		return models.ReviewResponse{}, fmt.Errorf("failed to check if product is reviewed: %w", err)
	}
	if isReviewed {
	...
	if err != nil {
		return models.ReviewResponse{}, fmt.Errorf("failed to create review: %w", err)
	}
