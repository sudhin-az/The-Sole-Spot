...
func (ad *AdminUseCase) GetOrderDetails(userID int) ([]models.FullOrderDetails, error) {

	fullOrderDetails, err := ad.adminrepository.GetOrderDetails(userID)
	if err != nil {
...
func (ad *AdminUseCase) CancelOrders(orderID string, userID int) error {
	...
	userTest, err := ad.adminrepository.UserOrderRelationship(orderIDInt, userID)
	if err != nil {
		return err
	}
	if userTest != userID {
	...
	// Begin a transaction
	tx, err := ad.adminrepository.BeginTransaction()
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	}
	defer ad.adminrepository.RollbackTransaction(tx)

	orderProductDetails, err := ad.adminrepository.GetProductDetailsFromOrders(orderIDInt)
	if err != nil {
		return err
	}
	...
	for _, product := range orderProductDetails {
		availableStock, err := ad.adminrepository.GetProductStock(product.ProductID)
		if err != nil {
			return err
		}
		...
		err = ad.adminrepository.UpdateProductStock(tx, product.ProductID, newStock)
		if err != nil {
	...
	err = ad.adminrepository.CommitTransaction(tx)
	if err != nil {
