...
func (h *UserHandler) ForgotPassword(c *gin.Context) {
	...
	userPassword, err := h.userUseCase.ForgotPassword(email, input)
	if err != nil {
		errRes := response.ClientResponse(http.StatusNotFound, "Failed to forgot password", nil, err.Error())
		c.JSON(http.StatusNotFound, errRes)
		return
	}
	successRes := response.ClientResponse(http.StatusOK, "Forgot password request processed successfully", userPassword, nil)
	c.JSON(http.StatusOK, successRes)
}
