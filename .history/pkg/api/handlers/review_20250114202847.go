...
func (r *ReviewHandler) AddReview(c *gin.Context) {
	...
	if err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "cannot create reviews", nil, err)
		c.JSON(http.StatusBadRequest, errorRes)
		return
	}
	successRes := response.ClientResponse(http.StatusOK, "Review Added Successfully", nil, nil)
...
func (r *ReviewHandler) GetAverageRating(c *gin.Context) {
	...
	successRes := response.ClientResponse(http.StatusOK, "avg rating retrieved Successfully", avgRating, nil)
	c.JSON(http.StatusOK, successRes)
}
