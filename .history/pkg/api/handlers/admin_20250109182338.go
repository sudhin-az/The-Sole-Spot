...
func (ad *AdminHandler) ListOrders(c *gin.Context) {
	adminName, ok := c.Get("admin_name")
	if !ok {
		errRes := response.ClientResponse(http.StatusUnauthorized, "Admin name not found in context", nil, nil)
		c.JSON(http.StatusUnauthorized, errRes)
		return
	}

	// Ensure the extracted value is a string
	name, ok := adminName.(string)
	if !ok {
		errRes := response.ClientResponse(http.StatusUnauthorized, "Invalid admin name format", nil, nil)
		c.JSON(http.StatusUnauthorized, errRes)
		return
	}

	// Pass the admin name to the use case or handle it as needed
	fullOrderDetails, err := ad.adminUseCase.GetOrderDetails(name) // Assuming GetOrderDetails accepts a string
	if err != nil {
...
func (ad *AdminHandler) AdminCancelOrders(c *gin.Context) {
	...
	if orderID == "" {
		errRes := response.ClientResponse(http.StatusBadRequest, "ID is required", nil, nil)
		c.JSON(http.StatusBadRequest, errRes)
		return
	}

	ID, ok := c.Get("admin_name")
	if !ok {
	...
	err := ad.adminUseCase.CancelOrders(orderID, userid)
	if err != nil {
