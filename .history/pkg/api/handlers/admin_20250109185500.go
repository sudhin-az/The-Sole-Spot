...
func (ad *AdminHandler) ListOrders(c *gin.Context) {
	...
	// Ensure the extracted value is an integer
	id, ok := userID.(int)
	if !ok {
	...
	fmt.Println("Admin ListOrders - userID:", id)

	// Fetch order details using the use case
	fullOrderDetails, err := ad.adminUseCase.GetOrderDetails(id)
	if err != nil {
