...
func (ad *AdminHandler) ChangeOrderStatus(c *gin.Context) {
	...
	if err := c.BindJSON(&input); err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "Field provided are in wrong format", nil, err.Error())
		c.JSON(http.StatusBadRequest, errorRes)
		return
	}
	orderstatus, err := ad.adminUseCase.ChangeOrderStatus(orderID, input.Status)
	if err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "Failed to update order status", nil, err.Error())
		c.JSON(http.StatusBadRequest, errorRes)
		return
	}
	successRes := response.ClientResponse(http.StatusOK, "Order status updated successfully", orderstatus, nil)
	c.JSON(http.StatusOK, successRes)
}
