...
func (o *OrderHandler) OrderItemsFromCart(c *gin.Context) {
	...
	paymentMe_id, _ := c.Get("paymentmethod_id")
	straddress := c.Get("address_id")
	product_id := c.Get("product_id")
	quantity := c.Get("address_id")
	...
	if err := c.ShouldBindJSON(&cartItems); err != nil {
		if singleErr := c.ShouldBindJSON(&cartItems); singleErr != nil {
			errorRes := response.ClientResponse(http.StatusBadRequest, "Invalid input data", cartItems, err.Error())
