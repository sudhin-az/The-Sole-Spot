...
func (ad *AdminHandler) ListOrders(c *gin.Context) {
	// Get the user_id from the context (it should be an integer)
	...
	// Ensure the extracted value is an int
	id, ok := userID.(int)
	if !ok {
	...
	fullOrderDetails, err := ad.adminUseCase.GetOrderDetails(id) // Passing the userID (int) to GetOrderDetails
	if err != nil {
