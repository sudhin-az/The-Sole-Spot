...
func (o *OrderHandler) OrderItemsFromCart(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&orderRequest); err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "Invalid input data", nil, err.Error())
	...
	order := models.Order{
		UserID:        userID.(int),
		AddressID:     uint(orderRequest.AddressID),
		PaymentMethod: orderRequest.PaymentMethod,
	}

	// Fetch Cart Items for the User
	cartItems, err := o.orderUseCase.FetchCartItemsForUser(order.UserID)
	if err != nil {
		errorRes := response.ClientResponse(http.StatusBadRequest, "Cart is empty; cannot place order", nil, err)
		c.JSON(http.StatusBadRequest, errorRes)
		return
	}
