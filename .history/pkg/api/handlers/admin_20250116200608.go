...
func (ad *AdminHandler) ChangeOrderStatus(c *gin.Context) {
	...
	var input struct {
		OrderStatus string `json:"status" binding:"required"`
	}
	...
	order, err := ad.adminUseCase.ChangeOrderStatus(orderID, input.Status)
	if err != nil {
	...
	successRes := response.ClientResponse(http.StatusOK, "Order status updated successfully", order, nil)
	c.JSON(http.StatusOK, successRes)
}
