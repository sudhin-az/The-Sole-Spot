...
func NewServerHTTP(userHandler *handlers.UserHandler, authHandler *handlers.AuthHandler,
	...
	cartHandler *handlers.CartHandler, orderHandler *handlers.OrderHandler) *ServerHTTP {
	router := gin.New()

	// Set up user routes
	userGroup := router.Group("/user")
	routes.UserRoutes(userGroup, userHandler, cartHandler, orderHandler, productHandler)
