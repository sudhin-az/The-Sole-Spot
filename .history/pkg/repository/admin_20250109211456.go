...
func (ar *AdminRepository) AdminOrderRelationship(orderID, userID int) (int, error) {
	...
	err := ar.DB.Raw(query, orderID).Scan(&associatedUserID).Error
	if err != nil {
		return 0, fmt.Errorf("failed to verify order relationship: %w", err)
	}
...
func (ad *AdminRepository) GetAllOrderDetails() ([]models.FullOrderDetails, error) {
	...
	err := ad.DB.Raw("SELECT order_id, final_price, shipment_status, payment_status FROM orders").Scan(&orderDetails).Error
	if err != nil {
