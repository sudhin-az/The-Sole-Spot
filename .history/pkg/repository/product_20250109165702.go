...
func (p *ProductRepository) GetProductsByCategory(categoryID string, sortBy string) ([]models.ProductResponse, error) {
	var products []models.ProductResponse
	query := p.DB.Model(&models.ProductResponse{}).Where("category_id = ? AND  stock > 0", categoryID)
