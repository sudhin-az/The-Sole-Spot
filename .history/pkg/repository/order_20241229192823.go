...
func (o *OrderRepository) GetOrderDetails(userID int) ([]models.FullOrderDetails, error) {
	...
	for _, od := range orderDetails {
		...
		o.DB.Raw("select order_items.product_id,order_items.product_name, order_items.quantity, order_items.total_price from order_items inner join products on order_items.product_id = products.id where order_items.order_id = ?", od.OrderId).Scan(&orderProductDetails)
