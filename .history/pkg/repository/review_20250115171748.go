...
func (r *ReviewRepository) DoesProductExist(productID string) (bool, error) {
	var count int64
	err := r.DB.Raw("SELECT COUNT(*) FROM reviews WHERE product_id = ?", productID).Scan(&count).Error
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func (r *ReviewRepository) DeleteReview(reviewID string) error {
	err := r.DB.Where("id = ?", reviewID).Delete(&domain.Review{}).Error
	if err != nil {
		return err
	}
	return nil
