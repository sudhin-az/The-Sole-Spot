...
func (o *OrderRepository) DoesCartExist(orderBody models.OrderFromCart, userID int) (bool, error) {
	var exist bool
	err := o.DB.Raw("select * from carts where user_id = ?)", userID).Scan(&exist).Error
	if err != nil {
		return false, err
	}
	return exist, nil
...
func (o *OrderRepository) AddressExist(orderBody models.OrderFromCart) (bool, error) {
	var count int
	err := o.DB.Raw("select count(*)from addresses where id=?", orderBody.AddressID).Scan(&count).Error
	if err != nil {
		return false, err
	}
	return count > 0, nil
