...
func (o *OrderRepository) GetOrderDetails(userID int) ([]models.FullOrderDetails, error) {
	...
	if err := o.DB.Raw("SELECT order_id, final_price, shipment_status, payment_status where user_id = ?", userID).Scan(&orderDetails).Error; err != nil {
		return nil, err
	}
	...
	for _, op := range orderDetails {
		...
		if err := o.DB.Raw(`SELECT order_items.product_id, order_items.product_name, order_items.quantity, order_items.total_price FROM order_items
