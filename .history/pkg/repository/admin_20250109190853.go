...
func (ad *AdminRepository) Getorderdetails(orderID int) ([]models.FullOrderDetails, error) {
	var orderDetails []models.OrderDetails

	// Fetch orders for the given user_id
	err := ad.DB.Raw("SELECT final_price, shipment_status, payment_status FROM orders WHERE order_id = ?", orderID).Scan(&orderDetails).Error
	if err != nil {
		fmt.Println("Error fetching order details:", err) // Debugging
		return nil, err
	}

	// Log the fetched order details
	fmt.Println("Fetched order details for user_id:", orderID, orderDetails)

	if len(orderDetails) == 0 {
		fmt.Println("No orders found for user_id:", orderID) // Debugging
		return nil, nil
	}
	...
	for _, od := range orderDetails {
		...
		if err != nil {
			fmt.Println("Error fetching product details for order_id:", od.OrderId, "Error:", err) // Debugging
			return nil, err
		}

		fmt.Println("Product details for order_id", od.OrderId, ":", orderProductDetails) // Debugging
...
func (ad *AdminRepository) ChangeOrderStatus(orderID string) (models.Order, error) {
	var shipment_status models.Order

	err := ad.DB.Raw("UPDATE orders set shipment_status = ? WHERE order_id = ?", orderID).Scan(&shipment_status).Error
	if err != nil {
		return models.Order{}, err
	}
	return shipment_status, nil
}
