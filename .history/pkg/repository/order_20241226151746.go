...
func (o *OrderRepository) AddressExist(orderBody models.OrderIncoming) (bool, error) {
    ...
    `
	err := o.DB.Raw(query, orderBody.AddressID, orderBody.UserID).Scan(&count).Error
	if err != nil {
		return false, err
	}
	return count > 0, nil
...
func (repo *OrderRepository) CheckStock(productID int, requiredQuantity int) (bool, error) {
	...
	query := `
        ...
        WHERE id = ?;
    `
	err := repo.DB.Raw(query, productID).Scan(&stockQuantity).Error
	if err != nil {
		return false, err
	}
	return stockQuantity >= requiredQuantity, nil
}

func (o *OrderRepository) UpdateProductStock(productID int, newStock int) error {
	return o.DB.Model(&domain.Products{}).Where("id = ?", productID).Update("stock", newStock).Error
}

func (o *OrderRepository) CreateOrder(orderDetails models.Order) error {
