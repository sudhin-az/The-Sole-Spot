...
func (o *OrderRepository) DoesCartExist(orderBody models.OrderFromCart, userID int) (bool, error) {
	var exist bool
...
func (o *OrderRepository) GetCartItems(userID int) ([]models.OrderFromCart, error) {
	var cartItems []models.OrderFromCart
	err := o.DB.Raw("SELECT product_id, quantity, price FROM carts WHERE user_id = ?", userID).Scan(&cartItems).Error
	return cartItems, err
}

func (o *OrderRepository) AddressExist(addressID uint) (bool, error) {
	var exist bool
	err := o.DB.Raw("SELECT EXISTS(SELECT 1 FROM addresses WHERE id = ?)", addressID).Scan(&exist).Error
	return exist, err
}

func (o *OrderRepository) GetProductStock(ProductID int) (int, error) {
	fmt.Println("productID", ProductID)
	var stock models.ProductResponse
...
func (o *OrderRepository) UpdateProductStock(tx *gorm.DB, productID int, newStock int) error {
	return o.DB.Model(&domain.Products{}).Where("id = ?", productID).Update("stock", newStock).Error
}

func (o *OrderRepository) CreateOrder(tx *gorm.DB, orderDetails models.Order) (string, error) {
	result := o.DB.Create(orderDetails)
	if result.Error != nil {
		return "", result.Error
	}
...
func (o *OrderRepository) GetOrderDetails(userID int) ([]models.FullOrderDetails, error) {
	...
	for _, od := range orderDetails {
		...
		o.DB.Raw("select order_items.product_id,products.name, order_items.quantity, order_items.total_price from order_items inner join products order_items.product_id = products.id where order_items.order_id = ?", od.OrderId).Scan(&orderProductDetails)
