...
func (car *CartRepository) AddToCart(userID int, productID int, price float64) error {
	var cartItem models.Cart
	...
	if err != nil {
		...
		return car.DB.Create(&newCart).Error
	}
...
func (car *CartRepository) RemoveProductFromCart(userID int, productID int, price float64) error {
	...
	// If quantity > 1, decrement the quantity and update the total price
