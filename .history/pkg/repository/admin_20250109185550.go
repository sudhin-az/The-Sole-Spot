...
func (ad *AdminRepository) Getorderdetails(userID int) ([]models.FullOrderDetails, error) {
	...
	err := ad.DB.Raw("SELECT order_id, final_price, shipment_status, payment_status FROM orders WHERE user_id = ?", userID).Scan(&orderDetails).Error
	if err != nil {
	...
	fmt.Println("Fetched order details for user_id:", userID, orderDetails)

	if len(orderDetails) == 0 {
		fmt.Println("No orders found for user_id:", userID) // Debugging
		return nil, nil
	}
